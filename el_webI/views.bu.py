...
def switch_on(request):
    try:
        comp_1 = get_object_or_404(Component_data,pk=1)
        ...
        print ('Connection failed!')
    #status_data = get_object_or_404(Component_data, pk=pk)
...
class component_status_list(APIView):
    ...
    def get(self,request):
        status = Component_data.objects.all()
        serializer = Component_status_Serializer(status, many=True)
        return Response(serializer.data)

    def put(self,request):
        comp = get_object_or_404(Component_data,pk=1)
